...
<div class="flex justify-between items-start">
        ...
        <div class="p-8 mx-auto max-w-5xl lg:py-16" id="cetak-nota">
          ...
          <div class="flex justify-center items-center space-x-4">
            <div class="text-center">
              <h2 class="mb-0 text-xl font-bold text-gray-900 dark:text-white">Laporan Produk Terjual</h2>
          ...
          <table id="laporan-produk-terjual" class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
              ...
              <tbody>
                  ...
                  @foreach ($sales as $sale)
                      <tr class="border-b dark:border-gray-700">
                          ...
                          <td class="px-4 py-3">{{ $sale->kode_produksi }}</td>
    ...
    <script>
        let filteredSales = @json($sales); // Menyimpan semua sale sebagai default
        ...
        function filterData() {
          ...
          // Lakukan filter data
          filteredSales = @json($sales).filter(sale => {
              const saleDate = new Date(sale.created_at);
              return saleDate >= new Date(startDate) && saleDate <= endDateAdjusted;
          });
        ...
        function updateTable(filteredSales) {
            ...
            filteredSales.forEach((sale, index) => {
                ...
                row.innerHTML = `
                    ...
                    <td class="px-4 py-3">${sale.product.name}</td>
                    <td class="px-4 py-3">Rp.${sale.harga_jual}</td>
                    <td class="px-4 py-3">${sale.stok_keluar}</td>
                    <td class="px-4 py-3">Rp.${sale.jumlah_harga_jual}</td>
        ...
        function exportToExcel() {
            ...
            const data = [
                ...
                [`{{ auth()->user()->username }}`],
                [],
                ["No.", "Nama Produk", "Harga Jual", "Qty Terjual", "Jumlah Harga Jual", "Tanggal"]
            ];
            ...
            filteredSales.forEach((sale, index) => {
                data.push([
                    index + 1, // Nomor urut
                    sale.product.name,
                    parseInt(sale.harga_jual),
                    sale.stok_keluar,
                    parseInt(sale.jumlah_harga_jual),
                    new Date(sale.created_at).toLocaleDateString('id-ID'), // Tanggal sale
            ...
            // Simpan file Excel
            XLSX.writeFile(workbook, "Laporan_Produk_Terjual.xlsx");
        }
    </script>
    <script>
      document.getElementById('download-pdf').addEventListener('click', async () => {
        ...
        // Unduh file PDF
        pdf.save('laporan_produk_terjual.pdf');
      });
    </script>
    ...
    <script>
        $(document).ready(function () {
            ...
            var table = $('#laporan-produk-terjual').DataTable({
                pageLength: 5,
                language: {
    ...
    $('#exportExcelButton').on('click', function () {
        ...
        // Ambil header tabel
        const header = ['No.', 'Kode Produksi', 'Nama Barang', 'Harga Jual', 'Jumlah Terjual', 'Jumlah Harga', 'Tanggal Terjual'];
        rows.push(header);
        ...
        filteredRows.each(function (data, index) {
            ...
            const hargaJual = parseInt(data[3].replace(/\D/g, ''));
            const jumlahTerjual = data[4];
            const jumlahHarga = parseInt(data[5].replace(/\D/g, ''));
            const tanggalTerjual = data[6];

            // Tambahkan baris dengan nomor ur, namaBarang, dan hargaJual sebagai integer
            rows.push([
